...
(function () {
  ...
  function card(v) {
    ...
    return '<article class="' + cls + '" data-id="' + v.id + '">' +
      ...
      '<div class="v-card__body">' +
        '<h3 class="v-card__name"><a href="vehicle_2.html?id=' + v.id + '">' + title + '</a></h3>' +
        ...
        '</div>' +
        '<a href="vehicle_2.html?id=' + v.id + '" class="v-card__details">View Details <i class="bi bi-arrow-right arrow"></i></a>' +
      '</div>' +
    '</article>';
  }
  ...
  function renderChips() {
    ...
    if (state.mileageMin) chips.push(chip("Over " + state.mileageMin.toLocaleString("en-US") + " mi", "mileageMin", null));
  ...
  function buildTopSearch() {
    ...
    host.innerHTML =
      dualHTML("year", "Year", minYear, maxYear, 1) +
      ...
      dualHTML("price", "Price", P_LO, P_HI, 500) +
      dualHTML("mileage", "Mileage", 0, M_HI, 1000) +
    ...
    // dual-thumb range sliders (Year, Price, Mileage)
    $$("[data-dual]", host).forEach(function (d) {
      var key = d.getAttribute("data-dual");
      var lo = +d.getAttribute("data-lo"), hi = +d.getAttribute("data-hi");
      var mn = $("[data-dual-min]", d), mx = $("[data-dual-max]", d);
      var fill = $("[data-dual-fill]", d), loLbl = $("[data-dual-lo]", d), hiLbl = $("[data-dual-hi]", d);
      var fmt = fmtFor(key);
      function paint() {
        var a = +mn.value, b = +mx.value;
        fill.style.left = ((a - lo) / (hi - lo) * 100) + "%";
        fill.style.right = (100 - (b - lo) / (hi - lo) * 100) + "%";
        loLbl.textContent = fmt(a, false); hiLbl.textContent = fmt(b, b >= hi);
        mn.style.zIndex = (a > lo + (hi - lo) * 0.6) ? 5 : 3;  // keep the min thumb grabbable near the top end
      }
      function commit() {
        var a = +mn.value, b = +mx.value;
        if (key === "year") { state.yearMin = a > lo ? a : null; state.yearMax = b < hi ? b : null; }
        else if (key === "price") { state.priceMin = a > lo ? a : null; state.priceMax = b < hi ? b : null; }
        else { state.mileageMin = a > lo ? a : null; state.mileageMax = b < hi ? b : null; }
        apply();
      }
      mn.addEventListener("input", function () { if (+mn.value > +mx.value) mn.value = mx.value; paint(); commit(); });
      mx.addEventListener("input", function () { if (+mx.value < +mn.value) mx.value = mn.value; paint(); commit(); });
      d._els = { mn: mn, mx: mx, lo: lo, hi: hi, key: key, paint: paint };
      paint();
    });
  ...
  function syncTopSearch() {
    ...
    $$("[data-dual]", host).forEach(function (d) {
      var e = d._els; if (!e) return;
      if (e.key === "year") { e.mn.value = state.yearMin || e.lo; e.mx.value = state.yearMax || e.hi; }
      else if (e.key === "price") { e.mn.value = state.priceMin || e.lo; e.mx.value = state.priceMax || e.hi; }
      else { e.mn.value = state.mileageMin || e.lo; e.mx.value = state.mileageMax || e.hi; }
      e.paint();
    });
  ...
  grid.addEventListener("click", function (e) {
    ...
    if (act) {
      ...
      if (act.getAttribute("data-act") === "photos") { location.href = "vehicle_2.html?id=" + v.id; return; }
    ...
    if (mc) location.href = "vehicle_2.html?id=" + mc.getAttribute("data-id");
  });
